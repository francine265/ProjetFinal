        ...
        private void btAjoutTrajet_Click(object sender, RoutedEventArgs e)
        {
         ...
         //  try
           // {
            ...
            if (valide == 0)
            {
                Singleton.getInstance().AjouterTrajet(tbxVilleDepart.Text,tbxVilleArrive.Text, tbxheureDepart.Time.ToString(), tbxheureArrive.Time.ToString(), calandar.Date.Value.ToString("yyyyy-MM-dd"), p, tbxArret.Text,tbxEtat.Text,tbxNumConducteur.Text, np, np);
